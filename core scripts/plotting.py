...
def raster_plotter(
    selection_index,
    spike_times,
    spike_trial, # This is the full spike_trial array initially
    psico,
    sample_rate=30000,
    title="",
    path=None,
    ...
    """
    # Hold onto original times/trials before potential subsetting if needed later
    original_spike_times = spike_times
    original_spike_trial = spike_trial

    if selection_index is not None:
        # Subset the data based on the selection
        # IMPORTANT: Check if selection_index is empty *before* subsetting
        #            or handle potential empty arrays resulting from subsetting
        if len(selection_index) > 0:
             spike_times = original_spike_times[selection_index]
             spike_trial = original_spike_trial[selection_index]
        else:
             # If selection_index is empty, create empty arrays
             spike_times = np.array([])
             spike_trial = np.array([], dtype=int) # Match original dtype

    # --- Check if the (potentially subsetted) spike_trial is empty ---
    if spike_trial.size == 0:
        print("DEBUG: raster_plotter - 0 spikes selected, setting max_trial_spikes=1.")
        max_trial_spikes = 1 # Use 1 to allow np.full to work
        spikes_present = False
    else:
        # Calculate max_trial_spikes only if spikes are present
        max_trial_spikes = int(np.max(np.unique(spike_trial, return_counts=True)[1]))
        # Ensure max_trial_spikes is at least 1 for np.full
        max_trial_spikes = max(1, max_trial_spikes)
        spikes_present = True
    # --- End check ---

    # Continue with setup - use full psico for trial info and axes setup
    CLASS_SEP = 3  # Num. of empty spaces between classes.
    clases_all, clas_indx_all, clas_counts_all = np.unique(
        psico.Class, return_counts=True, return_index=True
    )
    n_trials = len(pd.unique(psico.Trial)) # Total number of trials from psico
    raster_array = np.full((n_trials, max_trial_spikes), np.nan) # Create raster based on total trials
    align_times_all = np.array(psico[a_dim])
    stim_intensity_all = np.array(psico[stim_dims].iloc[clas_indx_all]).astype(int)
    trials_all = np.array(psico.Trial, dtype=int)

    # Determine trial order based on full psico data
    if clas_order is None:
        sort_order = [
            -stim_intensity_all[:, i] for i in reversed(range(stim_intensity_all.shape[1]))
        ]
        stim_descending_orderer = np.lexsort(sort_order)
        clas_order = stim_descending_orderer

    # Get ordered classes, counts, and stim intensities from full data
    clases = clases_all[clas_order]
    clas_counts = clas_counts_all[clas_order]
    stim_intensity_labels = stim_intensity_all[clas_order] # Use this for labels

    # Create the order of actual trial numbers based on class order
    trial_order_indices = []
    for i in clas_order:
         # Find original indices in psico corresponding to this class
         indices_in_class = np.where(psico.Class == i + 1)[0]
         if indices_in_class.size > 0:
              trial_order_indices.extend(indices_in_class)

    # Fill raster array row by row based on the established trial order
    # but only using the *selected* spikes
    if spikes_present:
         # Convert selected spike times to seconds
         spike_times = spike_times / sample_rate

         current_raster_row = 0
         for trial_idx_in_psico in trial_order_indices:
              # Get the actual trial number and alignment time for this row
              actual_trial_num = psico.iloc[trial_idx_in_psico]['Trial']
              align_time = align_times_all[trial_idx_in_psico]

              # Find spikes *from the selection* that match this trial number
              selected_spikes_in_this_trial = spike_times[spike_trial == actual_trial_num]

              if selected_spikes_in_this_trial.size > 0:
                   # Align these specific selected spikes
                   aligned_selected_spikes = selected_spikes_in_this_trial - align_time
                   # Place them in the current raster row, avoiding overflow
                   num_spikes_to_plot = min(len(aligned_selected_spikes), max_trial_spikes)
                   raster_array[current_raster_row, :num_spikes_to_plot] = aligned_selected_spikes[:num_spikes_to_plot]

              current_raster_row += 1
              if current_raster_row >= n_trials: # Safety break if logic error
                  break
    # Else: raster_array remains NaN if spikes_present is False

    # Calculate line offsets based on total trials and ordered class counts
    line_offsets = np.arange(n_trials) + np.repeat(
        (np.arange(len(clases))) * CLASS_SEP, repeats=clas_counts
    )

    # Setup ticks/spans based on full psico data (psico.iloc[0])
    ...
    # Build the figure using the potentially NaN raster_array but full context
    fig = raster_builder(
        title,
        raster_array, # Will be NaN-filled if no spikes selected
        extraticks,
        extratick_pos,
        axvspan_pos,
        line_offsets, # Based on full trial structure
        stim_intensity_labels, # Based on full trial structure
        clas_counts, # Based on full trial structure
        CLASS_SEP,
        xlim=xlim,
        savepath=path,
    )
